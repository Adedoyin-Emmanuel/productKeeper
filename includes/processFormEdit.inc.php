...
if(!$file_transfer_status){
	...
	if(!empty($legit_product_name) AND !empty($legit_product_desc) AND !empty($legit_product_img) AND !empty($product_id)){
			$insert_data = $connectionModel->update_record($legit_product_name,$legit_product_desc,$legit_product_img,$product_id);

		if($connectionModel->conn->query($connectionModel->update_query_result ) == TRUE){
		...
		echo $legit_product_name;
		echo $legit_product_desc;
		echo $legit_product_img;
