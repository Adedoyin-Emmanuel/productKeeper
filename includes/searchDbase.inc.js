...
$(document).ready(($)=>{
	...
	setInterval(()=>{
		let $searchProduct = $("#searchProduct");
			...
			const $USER_INPUT = $searchProduct.val();
			...
			xhttp.onreadystatechange = ()=>{
				if(xhttp.readyState == 4 && xhttp.status == 200){
					...
					const $parentDataDiv = $("#product_search_data");
					...
					if(xhttp.responseText == ""){
						...
						$data_template = randomString($searchErrors);
				  		$parentDataDiv.html(`
				  			...
				  			<h6 class='text-capitalize text-center fs-6 text-danger'>

				  			


				  			<svg class="" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-wifi-off" viewBox="0 0 16 16">
			 ...
			 xhttp.open("GET",`includes/searchDbase.inc.php?q=${$USER_INPUT}`,true);
			 xhttp.send();
		})
